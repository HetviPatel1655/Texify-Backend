...
export class ProductsService implements BaseCrudService<ProductDto, CreateProductDto, UpdateProductDto, ProductListQuery> {
  ...
  async list(query: ProductListQuery, tenantId: string): Promise<RepositoryListResult<ProductDto>> {
    return this.productsRepository.list(query, tenantId);
  }

  async getById(id: string, tenantId: string): Promise<ProductDto | null> {
    return this.productsRepository.findById(id, tenantId);
  }
  ...
  async update(id: string, dto: UpdateProductDto, context: CrudContext): Promise<UpdateResult<ProductDto>> {
    const existing = await this.productsRepository.findById(id, context.tenantId);
  ...
  async remove(id: string, context: CrudContext): Promise<void> {
    ...
    await this.productsRepository.softDelete(id, context.tenantId);
  }
}
